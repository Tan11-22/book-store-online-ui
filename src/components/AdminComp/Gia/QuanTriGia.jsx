...
import { getSachVaGia } from '../../../context/QuanTriSach';
import { formatCurrency } from '../../../context/utility';
import ModalsCTGia from './ModalsCTGia';
function QuanTriGia() {

    const [search, setSearch] = useState('')
    const [data, setData] = useState([])
    const [openModalsCTGia,setOpenModalsCTGia] = useState(false)
    const [dataSelect, setDataSelect] = useState({'isbn':'','tenSach':''})
    const [refresh, setRefresh] = useState(false)
    useEffect (
        () => {
            const fetchData = async () => {
                try {
                  const result = await getSachVaGia(search);
                  setData(result.data)
                } catch (error) {
                  console.log(error)
                }
    ...
    const handleOpenCTGiaSach = (isbn,ten) => {
        setDataSelect({'isbn':isbn, 'tenSach':ten})
        setOpenModalsCTGia(true)
    }
  return (
    <div>
    <div className="relative flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-xl bg-clip-border">
        ...
        <div className="p-2 px-0 overflow-scroll h-[87vh]">
            ...
            <tbody>
            {
                ...
                <td className="p-4 border-b border-blue-gray-50">
                    ...
                    <img src={`http://localhost:8080/api/quan-ly-sach-service/hinh-anh/get?name=${val.tenAnh?val.tenAnh:"default.png"}`} alt={val.tenAnh}
                ...
                <td className="p-4 border-b border-blue-gray-50">
                    ...
                    {val.giaNhap?formatCurrency(val.giaNhap):"Đang cập nhật"}
                    </p>
                </td>
                <td className="p-4 border-b border-blue-gray-50">
                    <p className="block font-sans text-sm antialiased font-normal leading-normal text-black">
                    {val.giaBan?formatCurrency(val.giaBan):"Đang cập nhật"}
                    </p>
                </td>
                <td className="p-4 border-b border-blue-gray-50">
                    <p className="block font-sans text-sm antialiased font-normal leading-normal text-black">
                    {val.giaKhuyenMai?formatCurrency(val.giaKhuyenMai):"Đang cập nhật"}
                    </p>
                </td>
                {/* <td className="p-4 border-b border-blue-gray-50">
                    {val.tenNhaXuatBan}
                ...
                <td className="p-4 border-b border-blue-gray-50">
                    ...
                    type="button"
                    onClick={()=>handleOpenCTGiaSach(val.isbn,val.tenSach)}
                    >
        ...
        <ModalsCTGia open={openModalsCTGia} onClose={()=>setOpenModalsCTGia(false)} data={dataSelect}  refresh={()=>setRefresh(!refresh)}/>
</div>
  )
}

export default QuanTriGia
