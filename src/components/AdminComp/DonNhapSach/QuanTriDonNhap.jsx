...
import { huyDonNhap, layDSDonNhapQT, layDSNXBQT } from '../../../context/QuanTriSach';
import ModalsTaoDonNhap from './ModalsTaoDonNhap';
import ModalsChiTietDonNhap from './ModalsChiTietDonNhap';
import ModalsTaoPhieuNhap from './ModalsTaoPhieuNhap';
import ConfirmForm from '../../Form/ConfirmForm';

function QuanTriDonNhap() {
    const [dataDon,setDataDon] = useState([])
    const [openModalsTaoDonNhap,setOpenModalsTaoDonNhap] = useState(false)
    const [openModalsCT,setOpenModalsCT] = useState(false)
    const [openModalsTaoPN,setOpenModalsTaoPN] = useState(false)
    const [idDonClick, setIdDonClick] = useState(0)
    const [dataNXB, setDataNXB] = useState([])
    const [refresh, setRefresh] = useState(false)

    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [idDonXoa, setIdDonXoa] = useState(0)

    const handleOpenDialog = (idDon) => {
        setIdDonXoa(idDon)
    ...
    const handleCloseDialog = () => {
        const fetchData = async () => {
            try {
              const result = await huyDonNhap(idDonXoa);
              if( result.code === 200) {
                setRefresh(!refresh)
              }
            } catch (error) {
    ...
    const handleOpenModalsCT = (id) =>{
        setIdDonClick(id)
        setOpenModalsCT(true)
    }
    const handleOpenModalsTaoPN = (id) =>{
        setIdDonClick(id)
    ...
    useEffect (
        () => {
            const fetchData = async () => {
                try {
                  const result = await layDSDonNhapQT();
  ...
  return (
    <div>
      <div className="relative flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-xl bg-clip-border">
            ...
            <div className="p-2 px-0 overflow-scroll h-[87vh]">
                ...
                <tbody>
                {
                    ...
                    <td className="p-4 border-b border-blue-gray-50">
                        ...
                        className="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-lg text-center align-middle font-sans text-xs font-medium uppercase text-gray-900 transition-all hover:bg-gray-900/10 active:bg-gray-900/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        type="button"
                            onClick={()=>handleOpenModalsCT(val.idDonNhapSach)}
                        >
                        ...
                        className="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-lg text-center align-middle font-sans text-xs font-medium uppercase text-gray-900 transition-all hover:bg-gray-900/10 active:bg-gray-900/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        type="button"
                        onClick={()=> handleOpenDialog(val.idDonNhapSach)}>
                        <span className="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
            ...
            content={`Bạn có chắc chắn muốn xoá đơn nhập #${idDonXoa}`}
            isOpen={isDialogOpen}
            onClose1 = {()=>setIsDialogOpen(false)}
            onClose2={handleCloseDialog}
      />
    </div>
  )
}

export default QuanTriDonNhap
