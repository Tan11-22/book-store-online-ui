...
import { taoTaiKhoanNV } from '../../context/LoginService';
// import { dangKy } from '../../context/LoginService'
// import Alert from '../Alert/Alert';

function ModalsTaoTaiKhoan({open, onClose, refresh}) {
    const handleClose = () => {
        onClose()
      }
      ...
      const [QUYEN, setQUYEN] = useState(1)
    ...
    const handleDangKy= () => {
        ...
        const dataRequest = {
            ...
            'idQuyen': parseInt(QUYEN)
        }
        const fetchData = async () => {
            try {
              const result = await taoTaiKhoanNV(dataRequest);
              console.log(result)
              if(result.code === 200) {
                ...
                <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg min-w-[800px] ">
                    <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded-lg border border-black max-h-[80vh]  overflow-y-auto">
                        <div className="w-full flex justify-start text-gray-600 mb-3">
                            ...
                            <h1 className="text-black font-lg font-bold tracking-normal leading-tight mb-4 text-3xl pt-3 ml-4">Tạo tài khoản nhân viên</h1>
                        </div>
                        


                        <label for="name" className="text-black text-sm font-bold leading-tight tracking-normal">Mã nhân viên(*)</label>
                        ...
                        pl-3 text-sm border-gray-300 rounded border" placeholder="Mã nhân viên" 
                        value={TDN}
                        onChange={(e) => setTDN(e.target.value)}
                        type='text'/>

                        <label for="name" className="text-black text-sm font-bold leading-tight tracking-normal">Vai trò</label>
                        <div>
                        <select
                            value={QUYEN}
...
export default ModalsTaoTaiKhoan
